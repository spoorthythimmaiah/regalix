...
Controller = (function() {
  ...
  Controller.prototype.renderView = function(data) {
    var searchKey;
    var root = this;
    var getSlicedData = function(data){
      return root.getOpt('limit') ? data.slice(0, root.getOpt('limit')) : data;
    };
    searchKey = this.getOpt("searchKey");
    data = this.callbacks("sorter").call(this, this.query.text, getSlicedData(data), searchKey);
    return this.view.render(getSlicedData(data));
  };
...
View = (function() {
  ...
  View.prototype.render = function(list) {
    ...
    for (i = 0, len = list.length; i < len; i++) {
      ...
      /** 
      * custom code - To group list based on playlist 
      * Add new label for each playlist title and 
      * insert demo to respective playlist based on playlist_id 
      */
      if(this.$el.find("#playlist"+item.playlist_id).length) {
        $li.insertAfter(this.$el.find("#playlist"+item.playlist_id));
      } else {
        var label = $(`<label id=playlist${item.playlist_id}>${item.playlist_name}</label>`);
        $ul.append(label);
        $li.insertAfter(this.$el.find("#playlist"+item.playlist_id));
      }
